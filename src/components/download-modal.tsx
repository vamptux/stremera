...
import { Download, Folder, Wifi, X, Zap } from 'lucide-react';
...
export function DownloadModal({
  ...
  useEffect(() => {
    ...
    api
      ...
      .catch((error) => {
        if (!active) return;
        toast.error('Failed to load default download path', {
          ...
          <div className='absolute bottom-4 left-6 z-20 flex items-end gap-4'>
            ...
            <div className='space-y-1 mb-1'>
              <div className='flex items-center gap-2'>
                <Badge
                  variant='outline'
                  className='bg-black/40 backdrop-blur-md border-white/10 text-xs'
                >
              ...
              <p className='text-zinc-400 text-sm max-w-md truncate'>{fileName}</p>
            </div>
          </div>
        </div>
